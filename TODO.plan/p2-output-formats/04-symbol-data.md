...
|---|---:|---:|---:|---:|
| `.` | 1,621 | 92,401 | 2,532 | 116,200 |
| `./core` | 1,621 | 92,401 | 2,532 | 116,199 |
| `./asciimath` | 471,791 | 476,368 | 472,403 | 501,661 |
| `./latex` | 121,525 | 149,563 | 122,028 | 195,717 |
| `./mathml` | 237,215 | 271,868 | 237,882 | 317,466 |
| `./unicodemath` | 209,013 | 203,118 | 209,483 | 248,298 |
...
(a string map, no XML layer) and `271,868` for `./mathml` (a descriptor map
...
2. From that clean commit and the clean pinned oracle, run the repository's
   ...
   That ancestry holds on the branch and can still be lost at merge time. A
   squash merge replaces both commits with one new commit, so the hash the
   manifests recorded stops existing on the target branch, and the pointer back
   to the tree that produced the data is gone — `gate-oracle.rb` blanks that
   field before diffing, correctly, so its byte comparison cannot see it
   either. Verified: the previously merged pair records a commit that
   `git merge-base --is-ancestor` rejects against `main`. So either the merge
   preserves both commits, or the data is regenerated after the merge. A gate
   in `test/gates/payload-validation.spec.ts` now fails on an orphaned
   `repository.commit` rather than leaving the requirement to memory.
...
`src/formatting/generated/`.

Run them to paths outside the repository and copy the results back. Each
generator excludes only its OWN output root from the dirty check, so running
them in sequence in place fails: the first writes `src/generated/`, which the
second then sees as unrelated dirt and refuses. `--allow-dirty` gets past that
and is the wrong answer, because it stamps `committable: false` on exactly the
output this step exists to produce. The shape that works, and the one
`oracle-repo-regeneration` itself uses:

```sh
tmp=$(mktemp -d); G=<clean oracle checkout>
export BUNDLE_GEMFILE=$G/Gemfile
bundle exec ruby scripts/generate-corpus.rb --gem $G \
  --out $tmp/corpus --symbols-out $tmp/generated
bundle exec ruby scripts/generate-core-data.rb --gem $G --out $tmp/core-generated
bundle exec ruby scripts/generate-formatting-data.rb --gem $G --out $tmp/formatting-generated
# review, then copy all four roots back
```

All three must run even when only one slice changed: the core and formatting
generators record `generate-corpus.rb`'s digest too, and so do both corpus
manifests. Measured — those runs change only the digest, clean and committable
lines; every other generated byte is identical. The `oracle-repo-regeneration` runner executes
