 ...
 *   for `sin`/`cos`/`tan`, when the argument lies so close to a multiple of
 *   `pi/2` that glibc's own range reduction is not accurate enough to trust
 *   (`LARGE_ARGUMENT`'s reduction guard).
 *
 * Each band is sized by `scripts/measure-libm-glibc-accuracy.mjs` the way
 * `pow.ts`'s was: the worst distance from the midpoint of any sampled input
 * glibc rounds the wrong way, times two, rounded up to a clean fraction.
...
/**
 * Each function's refusal band, `1/inverse` ULP (module header): the worst
 * glibc miss `scripts/measure-libm-glibc-accuracy.mjs` found over its
 * `uniform` and `human` samples, times two, rounded up to a clean fraction.
 * `TODO.plan/deferred.md`'s "Evaluation: `Math` function results inside
 * glibc's rounding band" records each function's figures; the worst misses
 * (seed `20260924`, glibc 2.35, Linux x86_64, measured 2026-09-24) were sin
 * 0.011730, cos 0.014086, tan 0.043039, asin 0.007770, acos 0.009629, atan
 * 0.012025, exp 0.005751 and log 0.003879 ULP from the midpoint.
 */
export const BANDS: Readonly<Record<LibmFunction, RoundingBand>> = {
  sin: band("sin", 40n, "1/40 (0.025)"),
  cos: band("cos", 32n, "1/32 (0.03125)"),
  tan: band("tan", 10n, "1/10 (0.1)"),
  asin: band("asin", 60n, "1/60 (0.0167)"),
  acos: band("acos", 50n, "1/50 (0.02)"),
  atan: band("atan", 40n, "1/40 (0.025)"),
  exp: band("exp", 80n, "1/80 (0.0125)"),
  log: band("log", 125n, "1/125 (0.008)"),
};

/**
 * The reduction guard for `sin`, `cos` and `tan`. Near a multiple of `pi/2`
 * the result is tiny (or, for `tan`, huge), and any absolute error in the
 * C library's reduced argument `r` becomes a relative error of about
 * `error / |r|` in the result — far beyond any band. Measured on the
 * hardest doubles for range reduction at every binary exponent (the
 * continued-fraction convergents of `pi/2`, down to `|r|` near `2^-61`,
 * `scripts/lib/libm-sample.mjs`'s `hard-reduction` category): glibc's
 * reduced-argument error reached `2^-102.3` for arguments up to `2^26.49`,
 * and `2^-91.7` from `2^26.79` (`115689413.36`, the smallest hard case it
 * missed by more than one ULP) up, where it missed by up to thousands of
 * ULP (`cos(6381956970095103 * 2^797)`, the double nearest a multiple of
 * `pi/2`, is 8 ULP out); `LARGE_ARGUMENT` splits the two below both, so the
 * gap between takes the wider guard. This refuses when `|r|`
 * is below `2^-SMALL_ARGUMENT_GUARD_BITS` (for `|x| < LARGE_ARGUMENT`) or
 * `2^-LARGE_ARGUMENT_GUARD_BITS` (above): each sits 16 times above the worst
 * measured error times the `2^63` that keeps its effect under 2^-10 ULP. Only
 * a reduced argument, not the first octant (`|x| < pi/4`, `k = 0`, where `r`
 * is `x` itself), and only a result computed from `sin r`, is guarded
 * (`guardReduction`); a uniformly random argument lands in the guard with
 * probability under `2^-24 / (pi/4)`, but the multiples of `pi` a person
 * types where the result is zero or a pole — `sin(pi)`, `cos(pi/2)`,
 * `tan(pi/2)` — land in it by construction. `cos(pi)` and `sin(pi/2)` do not.
 */
export const LARGE_ARGUMENT = 2 ** 26;
export const SMALL_ARGUMENT_GUARD_BITS = 35n;
export const LARGE_ARGUMENT_GUARD_BITS = 24n;

function reductionRefusal(fn: "sin" | "cos" | "tan", bits: bigint): never {
  throw new UnsupportedFeatureError(
    "evaluate",
    `the argument lies within 2^-${bits} of a multiple of pi/2, where the platform C ` +
      `library's ${fn} loses accuracy to cancellation and Ruby's answer depends on it`,
  );
}
 ...
 * Where it is `cos r`, near `±1`, an error in `r` barely moves it: measured,
 * glibc missed none of the 1,971 hard cases on those branches.
 */
function guardReduction(fn: "sin" | "cos" | "tan", x: number, reduced: Reduced): void {
  ...
  if (magnitude < ONE >> bits) reductionRefusal(fn, bits);
}
...
function correctlyRoundedSin(x: number, rounding: RoundingBand | null = BANDS.sin): number {
  if (!Number.isFinite(x)) return Number.NaN;
  if (Math.abs(x) < TINY) return x;
  const reduced = reduce(Math.abs(x));
  if (rounding !== null) guardReduction("sin", Math.abs(x), reduced);
...
function correctlyRoundedCos(x: number, rounding: RoundingBand | null = BANDS.cos): number {
  if (!Number.isFinite(x)) return Number.NaN;
  if (x === 0) return 1;
  const reduced = reduce(Math.abs(x));
  if (rounding !== null) guardReduction("cos", Math.abs(x), reduced);
  const { r, quadrant } = reduced;
  const { sin, cos } = sinCos(r);
  return roundFixed([cos, -sin, -cos, sin][quadrant] as bigint, rounding);
}

function correctlyRoundedTan(x: number, rounding: RoundingBand | null = BANDS.tan): number {
  if (!Number.isFinite(x)) return Number.NaN;
  if (Math.abs(x) < TINY) return x;
  const reduced = reduce(Math.abs(x));
  if (rounding !== null) guardReduction("tan", Math.abs(x), reduced);
  const { r, quadrant } = reduced;
  const { sin, cos } = sinCos(r);
...
function correctlyRoundedAsin(x: number, rounding: RoundingBand | null = BANDS.asin): number {
  if (Number.isNaN(x) || Math.abs(x) < TINY) return x;
  return roundFixed(fixedAsin(toFixed(x)), rounding);
}

function correctlyRoundedAcos(x: number, rounding: RoundingBand | null = BANDS.acos): number {
  if (Number.isNaN(x)) return x;
  if (x === 1) return 0;
  return roundFixed((piFixed(W) >> 1n) - fixedAsin(toFixed(x)), rounding);
}

function correctlyRoundedAtan(x: number, rounding: RoundingBand | null = BANDS.atan): number {
  if (Number.isNaN(x) || Math.abs(x) < TINY) return x;
 ...
 * computing.
 */
function correctlyRoundedExp(x: number, rounding: RoundingBand | null = BANDS.exp): number {
  if (Number.isNaN(x)) return x;
  if (x === 0) return 1;
...
function correctlyRoundedLog(x: number, rounding: RoundingBand | null = BANDS.log): number {
...
export const CORRECTLY_ROUNDED: Readonly<
  Record<LibmFunction, (x: number, rounding?: RoundingBand | null) => number>
> = {
