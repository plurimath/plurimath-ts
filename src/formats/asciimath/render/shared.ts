/**
 * Shared semantics for the per-kind AsciiMath render files (`./*.ts`): the
 * render context, the child-render contract, and the Ruby idioms more than one
 * gem class leans on. The per-kind files carry the per-class measured pins;
 * this file carries the cross-cutting ones (PORTING-STANDARDS.md — every
 * behaviour here was read off live gem instances against the pinned oracle,
 * plurimath 0.11.6, 00c52783).
 *
 * Layout contract (ARCHITECTURE.md §5, "How this maps to the gem"): one file
 * per node kind under `./`, each mirroring the gem file of the class it
 * renders; the dispatch table in `./index.ts` is typed total over `NodeKind`;
 * recursion goes through `context.render`, never through a direct import of
 * the table — that keeps the module graph acyclic (`no-circular` is an error
 * in the boundary gate).
 */
...
export interface RenderContext {
   ...
   * table (`./index.ts`) and renders it under THIS context. Returns `null`
 ...
 * here. Returns `null` only where Ruby returns nil (see `./font-style.ts`).
 */
