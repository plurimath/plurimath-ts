/**
 * Shared semantics for the per-kind LaTeX render files (`./*.ts`): the render
 * context, the child-render contract, and the Ruby idioms more than one gem
 * class leans on. The per-kind files carry the per-class measured pins; this
 * file carries the cross-cutting ones (PORTING-STANDARDS.md — every behaviour
 * here was read off live gem instances against the pinned oracle, plurimath
 * 0.11.6, 00c52783).
 *
 ...
 * (`./binary-function.ts`), `Tr` (`./unary-function.ts`) and the table
 * column descriptors (`./table.ts`).
 */
