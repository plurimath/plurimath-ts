 ...
 * option's default-symbol behavior: decimal/group markers, integer-side
 * digit grouping, and fraction-side digit grouping. This is the shape, not
 * the content: it exists so a format's grammar and renderer can take these
 * as parameters instead of reaching for a global, and so the later work is
 * filling in data rather than restructuring rules.
 *
 * **What is not here, on purpose:** precision, significant digits, notation,
 * base notation, sign handling, currency, and the rest of the Ruby
 * `Formatter::Numbers` port — see `number-format.ts`'s header for exactly
 * what this slice does and does not cover. ARCHITECTURE.md §9 puts the rest
 * in P4, and §10 lists `formatting` as "minimal normalization in P1, locales
 * and configurable formatters in P4+".
 *
