 ...
 *      statement of what the sweep must contain. Every row is checked against it, and the
 *      fixture's own `rowCount`/`rendersCount` are checked against its own rows.
 *      A reviewer cut the fixture from 196 rows to ONE, left `rowCount: 196`,
 *      and both specs still passed 94 tests. Nothing read that number.
 ...
 *      `DEGENERATE_REFUSES` with a reason, and is pinned as refusing.
 */
...
import { DEGENERATE_REFUSES, FORMAT, NODE_FOR, RENDER } from "./parity-target";
