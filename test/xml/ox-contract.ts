 ...
 * `96bb34fc62`, Ox 2.14.28, Ruby 4.0.1) — never retyped by hand. The JSON is
 * regenerated by `scripts/generate-xml-fixtures.rb`, run from the oracle
 * checkout (invocation in its header); regeneration is deterministic and
 * byte-identical. Each fixture here rebuilds the same tree through `src/xml`
 * and names, in `probe`, the Ruby that produced its JSON entry, in two dump
 * spellings matching the two exported functions:
 *
 ...
 *   `math…-tree` entries are additionally byte-checked against real
 *   `Plurimath::Math.parse(...).to_mathml` output before the JSON is
 *   emitted (`scripts/generate-xml-fixtures.rb` aborts on mismatch).
 *
